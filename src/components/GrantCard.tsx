...
export function GrantCard({ grant }: GrantCardProps) {
  ...
  // Prioritér altid den officielle kilde-/informationsside til information om puljen
  const infoUrl = grant.sourceUrl || grant.foundation?.websiteUrl || '#';

  // Tjek om der findes en separat ansøgningsportal (forskellig fra kildesiden)
  const hasSeparateApplication = Boolean(
    grant.applicationUrl &&
    grant.applicationUrl.trim() !== '' &&
    grant.applicationUrl !== grant.sourceUrl
  );
  const applicationUrl = grant.applicationUrl;

  const isPortalLogin = applicationUrl?.includes('portal.slks.dk') ||
                        applicationUrl?.includes('ec.europa.eu/info/funding-tenders') ||
                        applicationUrl?.includes('norma.');
  ...
  return (
    <div className="nordic-card p-5 flex flex-col justify-between group hover:border-slate-300 transition-all">
      <div>
        ...
        {/* Title as clickable link directly to puljeinformation / guidelines */}
      ...
      <div className="pt-3 border-t border-slate-100/90 mt-2 space-y-3">
        ...
        {/* To adskilte handlinger for information og ansøgning */}
        <div className="pt-1 flex flex-col gap-1.5">
          {/* 1. Primær handling: Informations- og vejledningsside */}
          <a
            href={infoUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full inline-flex items-center justify-between px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold transition-all shadow-xs group/info"
            title="Læs fondens officielle vejledning, hvem der kan søge, og kriterier"
          >
            <span>Læs puljevejledning & krav</span>
            <ExternalLink className="w-3.5 h-3.5 text-slate-300 group-hover/info:translate-x-0.5 transition-transform" />
          </a>

          {/* 2. Sekundær handling: Direkte indsendelse/portal hvis særskilt */}
          {hasSeparateApplication && (
            <a
              href={applicationUrl!}
              target="_blank"
              rel="noopener noreferrer"
              className="w-full inline-flex items-center justify-between px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 rounded-lg text-[11px] font-medium border border-slate-200/80 transition-all group/app"
              title={isPortalLogin ? "Åbn den officielle ansøgningsportal (kræver MitID/login)" : "Gå direkte til digitalt ansøgningsskema"}
            >
              <span className="flex items-center gap-1.5">
                <span>Gå til ansøgningsportal</span>
                {isPortalLogin && (
              ...
              <ExternalLink className="w-3 h-3 text-slate-400 group-hover/app:translate-x-0.5 transition-transform" />
            </a>
          )}
        </div>
      </div>
